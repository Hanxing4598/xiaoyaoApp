	...
	<view class="content">
		...
		<form class="form" @submit="submit">
			<view class="item">
				...
				<input v-model="form.loginName" name="phone" placeholder="请输入登录名" />
			</view>
			<view class="item">
				<view class="icon cuIcon-people"></view>
				<input v-model="form.userName" name="phone" placeholder="请输入昵称" />
			</view>
			<view class="item">
				<view class="icon cuIcon-lock"></view>
				<input name="password" v-model="form.password" type="password" placeholder="请输入登录密码" />
				<!-- <view class="isShow">
			...
			<view class="item">
				<view class="icon cuIcon-lock"></view>
				<input name="password" v-model="form.repassword" type="password" placeholder="请确认登录密码" />
			</view>
...
<script>
	...
	import { register } from '@/request/interface/user'
	export default {
		data() {
			return {
				account: '',
		...
		methods: {
			...
			async submit({
				...
				if (code === 200) {
					...
					this.$alert.showToast({ title: msg })
				}
			}
		}
	}
